...
:
   ...
   { FindAcl(ACCESS_T, _localctx.name.getText()); }
;
...
:
   ...
   { FindAcl(COMMUNITY_T, _localctx.name.getText()); }
;
...
ip_extcommunity_list_stanza
:
   IP EXTCOMMUNITY_LIST ( name = DEC | name = VARIABLE ) ~NEWLINE* NEWLINE
   { FindAcl(COMMUNITY_T, _localctx.name.getText()); }
;
...
:
   ...
   { FindAcl(PREFIX_T, _localctx.name.getText()); }
;
